...
def unpack_message(dat):
	...
	metadata = np.array([first_idx,snr,samp_idx,time_since,h_group_filter_inds,h_group_dm_inds,h_group_dms,h_group_members,MJD,cand_MJD,cand_hour,cand_minute,cand_sec])
...
key = sysv_ipc.ftok("/home/user/linux_64/heimdall_buffer/progfile",65) #key of message queue to listen to
...
while reader.isConnected: #as long as the reader is connected...
	...
	for i in range(len(candidates)): #for each candidate, calculate what part of data to save and put it in an h5 file
		s1 = int(candidates[i][2]) - int(candidates[i][0]) - 1000
		s1 = s1*(s1 > 0)
		s2 = min(s1 + int(disp_delay(float(candidates[i][6]),dt,1.53,1.28)+1)+2**int(candidates[i][4]) + 1000,200000)
